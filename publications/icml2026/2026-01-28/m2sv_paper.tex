...
\begin{table}[t]
  ...
  \begin{tabular}{l r r}
    ...
    Human baseline & 200 & 95.0 $\pm$ 3.0 \\
    \bottomrule
  \end{tabular}
\end{table}

Human accuracy reaches 95\%, establishing a clear upper bound on task difficulty
and confirming that m2sv is reliably solvable by people despite real-world visual
noise and temporal mismatch. In contrast, even the strongest proprietary model
(Gemini-3-Pro) falls more than 30 points below human performance, while most open
models operate only marginally above the random baseline. This large and
consistent gap indicates that zero-shot pretrained VLMs lack robust mechanisms
for aligning overhead maps with egocentric street-level views.
...
required. To probe these effects directly, we analyze difficulty as perceived by a
human annotator, using response time as a proxy for cognitive effort.

During manual evaluation on a 200-example analysis subset, we record the elapsed
time required for a human to answer each question. Figure~\ref{fig:human-time-vs-symmetry}
...
human-perceived difficulty, defined by tertiles of response time. Humans maintain
near-ceiling performance across all buckets, with only a modest drop on the
hardest examples, indicating that longer response times reflect increased effort
rather than intrinsic ambiguity. In contrast, all evaluated models degrade sharply
with increasing difficulty and approach chance-level performance on the hardest
examples.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/time_difficulty_accuracy.png}
  \caption{
    \textbf{Accuracy vs.\ human-perceived difficulty.}
    Accuracy as a function of difficulty defined by human response time.
    Examples are partitioned into Easy, Medium, and Hard buckets by response-time
    tertiles.
...
Human evaluation in this work is conducted with a single participant due to time
and resource constraints at the time of data collection. While this limits the
ability to characterize inter-subject variability, the collected data provides a
useful reference point for comparing human and model behavior on structurally
ambiguous cases. To support future work, we will release both the interactive
labeling interface and the manually curated labels, including elapsed time
metadata from human annotation, to facilitate larger-scale human studies and
